...
%% time sync
if (parseImgsMetaData && ~sim)
    startTimes = [lfTime(1), rfTime(1), imuTime(1), allJointsTime(1), imgTime(1)];        
else
    startTimes = [lfTime(1), rfTime(1), imuTime(1), allJointsTime(1)];
end

if sim
    startTimes = [startTimes, simbaseTime(1)];
    startTimes = [startTimes, simlFootTime(1), simrFootTime(1)];
end

minStartTime = min(startTimes);
lfTime = lfTime - minStartTime;
rfTime = rfTime - minStartTime;
imuTime = imuTime - minStartTime;
allJointsTime = allJointsTime - minStartTime;

if (parseImgsMetaData && ~sim)
    imgTime = imgTime - minStartTime;
end

if sim
    simbaseTime = simbaseTime - minStartTime;
    simlFootTime = simlFootTime - minStartTime;
    simrFootTime = simrFootTime - minStartTime;
end
